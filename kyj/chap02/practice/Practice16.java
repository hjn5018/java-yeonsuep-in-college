package practice;

public class Practice16 {
	public static void main(String[] args) {
		int i1 = 0x11, i2 = 5;
		final int ONE = 1;
		char c1 = 'a';
		float f1 = 1.5f;
		double d1 = 2.8;
		boolean b1 = true;
//		System.out.printf("i1 = %d\n", i1); // 3(X) 17(O) <- 0x11은 이진수가 아닌 십육진수이다.
//		System.out.printf("i1/2 = %d\n", i1 / 2); // 8
//		System.out.println("c1 + ONE = " + c1 + ONE); // "c1 + ONE + a + 1" (X) "c1 + ONE + a1"(O)
//		System.out.println("(int)c1 + ONE " + (int) c1 + ONE); // "(int)c1 + ONE 991" (X) "(int)c1 + ONE 971" (O)
//		System.out.printf("(c1 + ONE) = %d\n", c1 + ONE); // 98
//		System.out.printf("(c1 + ONE) = %c\n", c1 + ONE); // b
//		System.out.printf("(c1 + ONE) = %5s\n", c1 + ONE); //     b (X)    98 (O)
//		System.out.printf("i2 + f1 = %f\n", i2 + f1); // 6.500000
//		System.out.printf("f1 + d1 = %.1f\n", f1 + d1); // 4.3
//		System.out.printf("(int)(f1 + d1) = %d\n", (int) (f1 + d1)); // 4
//		System.out.printf("(int)f1 + (int)d1 = %d\n", (int) f1 + (int) d1); // 3
//		System.out.println("b1 = " + b1); // b1 = true
	}
}
